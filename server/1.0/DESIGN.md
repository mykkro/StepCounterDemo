...
1. App calls `POST /api/devices/auth` → receives `{ "token": "...", "session_guid": "<UUID>", "expires_in": 3600 }`.
2. Server creates a `sensor_sessions` row and embeds `session_guid` in the JWT.
3. All measurements submitted with this JWT are linked to that session row (for training plan pairing).
4. All subsequent measurement uploads include `Authorization: Bearer <token>`.
5. JWT payload: `{ sub: client_guid, device_guid, device_type, session_guid }`.
