...
1. **Authenticate** — calls `POST /api/devices/auth` with username, password, device GUID, and device type. Receives a JWT. The device is auto-registered on first use.
